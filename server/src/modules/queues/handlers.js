...
const uploadedHandler = async (job) => {
  logger.info('video uploaded handler - title:', job.data.title);
...
const processingHandler = async (job) => {
  logger.info('video processing handler - input path:', job.data.path);
...
const processedHandler = async (job) => {
  logger.info('post-processing handler - processed path:', job.data.path);
...
const hlsConvertingHandler = async (job) => {
  logger.info('HLS conversion handler - source:', job.data.path);
  ...
  logger.info('HLS conversion result:', hlsConverted);
  return;
};

const hlsConvertedHandler = async (job) => {
  logger.info('HLS converted handler - filename:', job.data.filename);
...
const notifyVideoHlsConvertedHandler = async (job) => {
  logger.info('notifyVideoHlsConvertedHandler - payload:', job.data);
